...
func TestPlaceBetHandler_ErrorMapping(t *testing.T) {
	...
	cases := []struct {
		...
		{"invalid outcome", bets.ErrInvalidOutcome, http.StatusBadRequest, string(handlers.ReasonInvalidOutcome)},
		...
		{"not found", dmarkets.ErrMarketNotFound, http.StatusNotFound, string(handlers.ReasonNotFound)},
	}
...
func TestPlaceBetHandler_AuthFailureMapping(t *testing.T) {
	t.Setenv("JWT_SIGNING_KEY", "test-secret-key-for-testing")
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			rr := httptest.NewRecorder()
			handler := PlaceBetHandler(betsSvc, tc.usersSvc)
			handler.ServeHTTP(rr, req)
